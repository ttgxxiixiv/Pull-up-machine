...
(function () {
  ...
  // Порядок дней для курсора
  function programDays(week) {
    const plan = planForWeek(week);
    const keys = state.settings.daysPerWeek === 4 ? ['volume', 'strength', 'variety', 'light'] : ['volume', 'strength', 'variety'];
    return keys.map((k) => plan.days.find((d) => d.key === k));
  }
  function dayByKey(week, key) { return planForWeek(week).days.find((d) => d.key === key); }

  function advanceCursor() {
    const n = programDays(state.cursor.week).length;
    let { week, day } = state.cursor;
    day += 1;
    if (day >= n) { day = 0; week += 1; }
    state.cursor = { week, day };
  }
  function clampCursor() {
    const n = programDays(state.cursor.week).length;
    if (state.cursor.day >= n) state.cursor.day = 0;
  ...
  function sessionsInLastDays(days, dateStr) {
    const from = addDays(dateStr || todayStr(), -(days - 1));
    return state.sessions.filter((s) => s.date >= from && s.date <= (dateStr || todayStr()));
  }
  ...
  function renderToday() {
    ...
    const { week, day } = state.cursor;
    const days = programDays(week);
    const cur = days[day];
    const info = weekInfo(week);
    const today = todayStr();
    const todays = state.sessions.filter((s) => s.date === today);
    const last7 = sessionsInLastDays(7);
    const lastAny = sortedSessions().slice(-1)[0];

    let warn = '';
    if (todays.length) warn += `<div class="callout good"><span class="ico">✓</span><span>Сегодня уже записана тренировка: ${esc(todays.map((s) => s.title).join(', '))}. Ещё одна — только если это лёгкий день.</span></div>`;
    else if (last7.length >= 4) warn += `<div class="callout warn"><span class="ico">!</span><span>За последние 7 дней уже ${last7.length} тренировки. Лучше отдохнуть: программа рассчитана на 3–4 занятия в неделю.</span></div>`;
    else if (lastAny && lastAny.date === addDays(today, -1) && lastAny.type !== 'light' && cur.key !== 'light') warn += `<div class="callout"><span class="ico">i</span><span>Вчера была тренировка «${esc(DAY_SHORT[lastAny.type])}». Между тяжёлыми днями нужен хотя бы день отдыха — или сделайте сегодня лёгкий день.</span></div>`;

    // Подсказка по прошлому выполнению этого дня
    let adapt = '';
    const prev = lastSessionOfType(cur.key);
    if (prev && cur.key !== 'light') {
      const ev = evaluate(prev);
      if (ev.fail) adapt = `<div class="callout warn"><span class="ico">↺</span><span>В прошлый раз (${fmtShort(prev.date)}) этот день не вышел полностью. Сегодня возьмите вариант с запасом: на 2 повторения меньше в каждом подходе, но все подходы чисто.</span></div>`;
      else if (ev.ok && ev.total) adapt = `<div class="callout good"><span class="ico">↑</span><span>Прошлый раз (${fmtShort(prev.date)}) выполнен полностью. Идём по плану.</span></div>`;
    }

    card(`
      <div class="card-head">
        <div>
          <div class="small">Неделя ${week} · блок ${info.block + 1}${info.deload ? ' · разгрузка' : ''} · ${state.settings.daysPerWeek} дн/нед</div>
          <h1>${esc(cur.title)}</h1>
        </div>
        <span class="pill${info.deload ? ' pill-muted' : ''}">${info.deload ? 'Разгрузка' : 'Тренировка ' + (day + 1) + ' из ' + days.length}</span>
      </div>
      ${warn}${adapt}
      <p style="margin-top:8px"><strong>${esc(cur.summary)}</strong></p>
      ${cur.exercises.map((e) => exerciseTargetHtml(e)).join('')}
      <ul class="tips">${cur.tips.map((t) => `<li>${esc(t)}</li>`).join('')}</ul>
      <div class="btn-row" style="margin-top:14px">
        <button class="btn" id="btn-start" type="button">Записать тренировку</button>
        <button class="btn btn-ghost" id="btn-skip" type="button">Пропустить день</button>
      </div>
    `);

    const other = planForWeek(week).days.filter((d) => d.key !== cur.key);
    const el = card(`
      <h2>Другая тренировка на этой неделе</h2>
      <p class="small">Например, если сегодня нет сил на тяжёлый день. Лёгкий день можно добавлять сверх плана.</p>
      <div class="day-list">
        ${other.map((d) => `<button class="day-item" type="button" data-key="${d.key}"><span><span class="day-title">${esc(d.title)}</span><br><span class="day-sub">${esc(d.summary)}</span></span><span>›</span></button>`).join('')}
      </div>
    `);
    $$('.day-item', el).forEach((b) => b.addEventListener('click', () => {
      const d = dayByKey(week, b.dataset.key);
      const idx = days.findIndex((x) => x.key === d.key);
      draft = newDraftFromDay(week, d, idx >= 0);
      draft.cursorDay = idx;
      saveDraft(); render();
    }));

    $('#btn-start').addEventListener('click', () => { draft = newDraftFromDay(week, cur, true); draft.cursorDay = day; saveDraft(); render(); });
    $('#btn-skip').addEventListener('click', () => { if (confirm('Пропустить «' + cur.title + '» и перейти к следующей тренировке?')) { advanceCursor(); save(); render(); toast('День пропущен'); } });

    if (lastAny) {
      card(`<h2>Последняя тренировка</h2><div class="small">${esc(fmtDate(lastAny.date))} · ${esc(lastAny.title)}</div>${sessionTableHtml(lastAny)}`);
    } else {
      card(`<h2>Как пользоваться</h2><ul class="tips"><li>Нажмите «Записать тренировку», внесите фактические повторения по подходам и сохраните.</li><li>Программа сама переходит к следующему дню. Пропустили — нажмите «Пропустить день».</li><li>Прогресс хранится на этом устройстве. Чтобы не потерять — делайте экспорт в разделе «Ещё».</li><li>Добавьте страницу на главный экран телефона: тогда дневник откроется и без интернета.</li></ul>`);
    }
  }
  ...
  function renderForm() {
    ...
    const el = card(`
      ...
      <div class="field-row">
        ...
        <div class="field"><label for="f-rest">Таймер отдыха, с</label><input id="f-rest" type="number" min="15" step="15" value="${d.restSec || 120}"></div>
      </div>
      <div class="field"><label for="f-notes">Заметки</label><textarea id="f-notes" placeholder="Как прошло, что болит, что поменять">${esc(d.notes || '')}</textarea></div>
      <div class="btn-row" style="margin-top:14px">
        <button class="btn" type="button" id="btn-save">Сохранить тренировку</button>
        <button class="btn btn-ghost" type="button" id="btn-timer">Таймер отдыха</button>
      </div>
    `);
    ...
    $('#btn-timer', el).addEventListener('click', () => startTimer(d.restSec || 120));
  ...
  function saveSession() {
    ...
    if (!filled) { toast('Внесите хотя бы один подход'); return; }
    const session = {
    ...
    if (!d.editing && d.program && d.week === state.cursor.week && d.cursorDay === state.cursor.day) {
      advanceCursor();
      msg += ' · следующая: ' + programDays(state.cursor.week)[state.cursor.day].title;
    }
  ...
  function renderDiary() {
    ...
    const head = card(`<div class="card-head"><h1>Дневник</h1><span class="pill pill-muted">${list.length} ${plural(list.length, 'тренировка', 'тренировки', 'тренировок')}</span></div>
      <div class="btn-row"><button class="btn btn-ghost btn-sm" id="btn-add-any" type="button">+ Записать другую тренировку</button></div>`);
    $('#btn-add-any', head).addEventListener('click', () => {
      const days = planForWeek(state.cursor.week).days;
      const choice = prompt('Какая тренировка? Введите номер:\n' + days.map((d, i) => `${i + 1} — ${d.title}`).join('\n'), '1');
      const i = parseInt(choice, 10) - 1;
      if (isNaN(i) || !days[i]) return;
      draft = newDraftFromDay(state.cursor.week, days[i], false); draft.cursorDay = -1; saveDraft(); currentTab = 'today'; render();
    });
    ...
    list.forEach((s) => {
      ...
      const ev = evaluate(s);
      const badge = s.type === 'light' ? '' : ev.total ? (ev.ok ? '<span class="pill pill-good">выполнено</span>' : ev.fail ? '<span class="pill pill-warn">с запасом</span>' : '<span class="pill pill-muted">частично</span>') : '';
      html += `<div class="session" data-id="${s.id}">
    ...
    $$('[data-delete]', el).forEach((b) => b.addEventListener('click', () => { if (confirm('Удалить эту тренировку?')) { state.sessions = state.sessions.filter((x) => x.id !== b.dataset.delete); save(); render(); toast('Удалено'); } }));
  }
  ...
  function sessionTableHtml(s) {
    ...
    return `<div class="session-body"><table>${s.exercises.map((e) => {
      ...
      const sets = e.sets.map((st) => `${st.v}${ex.unit === 'sec' ? ' с' : ''}${st.w ? '<span class="muted">' + fmtKg(st.w).replace('+', '+') + '</span>' : ''}`).join(' · ');
  ...
  function renderProgress() {
    ...
    if (!list.length) { card('<h1>Прогресс</h1><div class="empty">Графики появятся после первых тренировок.</div>'); return; }
    ...
    const weekKeys = [];
    if (byWeek.size) { let k = mondayOf(list[0].date); const end = mondayOf(todayStr()); while (k <= end && weekKeys.length < 60) { weekKeys.push(k); k = addDays(k, 7); } }
    ...
    tiles.innerHTML = `
      <div class="tile"><div class="label">Тренировок</div><div class="value">${list.length}</div><div class="delta">неделя ${state.cursor.week} программы</div></div>
  ...
  function renderSettings() {
    ...
    const el = card(`<h1>Программа</h1>
      ...
      <div class="field-row">
        ...
        <div class="field"><label for="s-day">Следующая тренировка</label><select id="s-day">${programDays(state.cursor.week).map((d, i) => `<option value="${i}"${i === state.cursor.day ? ' selected' : ''}>${esc(d.title)}</option>`).join('')}</select></div>
      </div>
      <div class="btn-row" style="margin-top:12px"><button class="btn btn-ghost btn-sm" type="button" id="btn-repeat">Повторить текущую неделю</button></div>`);
    $$('#seg-days button', el).forEach((b) => b.addEventListener('click', () => { s.daysPerWeek = +b.dataset.v; clampCursor(); save(); render(); }));
    ...
    $('#s-week', el).addEventListener('change', (e) => { state.cursor.week = Math.max(1, parseInt(e.target.value, 10) || 1); state.cursor.day = 0; planWeek = null; save(); render(); });
    $('#s-day', el).addEventListener('change', (e) => { state.cursor.day = +e.target.value; save(); });
    $('#btn-repeat', el).addEventListener('click', () => { state.cursor.day = 0; save(); render(); toast('Неделя ' + state.cursor.week + ' начнётся заново'); });
    ...
    const ex = card(`<h1>Сохранение и перенос</h1>
      ...
      <div class="btn-row">
        <button class="btn" type="button" id="btn-export">Скачать файл</button>
    ...
    $('#btn-export', ex).addEventListener('click', exportFile);
  ...
  async function shareFile() {
    ...
    } catch (e) { if (e.name !== 'AbortError') exportFile(); }
  }
  ...
  // Сохранение при уходе со страницы (на случай, если браузер выгрузит вкладку)
  window.addEventListener('pagehide', () => { save(); saveDraft(); });
