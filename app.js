...
(function () {
  ...
  function normalize(obj) {
    ...
    out.sessions.forEach((s) => { if (!s.id) s.id = uid(); s.exercises.forEach((e) => { if (!Array.isArray(e.sets)) e.sets = []; }); });
    return out;
  }
  function save() {
  ...
  // Подходы по плану для упражнения (когда блок возвращают после пропуска)
  function plannedSets(d, key) {
    const day = DAY_TITLES[d.type] ? dayByKey(d.week || state.cursor.week, d.type) : null;
    const e = day && day.exercises.find((x) => x.key === key);
    return e ? e.sets.map((st) => ({ v: st.v, w: st.w || 0, target: st.v })) : [{ v: null, w: 0, target: null }];
  }
  ...
  function renderFormExercises(wrap) {
    ...
    wrap.innerHTML = d.exercises.map((e, ei) => {
      ...
      if (e.skipped) {
        return `<div class="exercise skipped" data-ei="${ei}">
          <div class="exercise-head"><h3>${esc(e.name)}</h3><button type="button" class="btn btn-ghost btn-sm" data-unskip="${ei}">Вернуть</button></div>
          <div class="small">Блок пропущен${targetTxt ? ' · ' + esc(targetTxt) : ''}</div>
          <input type="text" class="skip-reason" placeholder="Причина, например: нет гантели" value="${esc(e.skipReason || '')}" data-reason="${ei}" aria-label="Причина пропуска">
        </div>`;
      }
    ...
    $$('[data-skip]', wrap).forEach((b) => b.addEventListener('click', () => {
      const e = d.exercises[+b.dataset.skip]; e.skipped = true; saveDraft(); renderFormExercises(wrap);
      const r = $(`[data-reason="${b.dataset.skip}"]`, wrap); if (r) r.focus();
    }));
    $$('[data-unskip]', wrap).forEach((b) => b.addEventListener('click', () => {
      const e = d.exercises[+b.dataset.unskip]; e.skipped = false;
      if (!e.sets.length) e.sets = plannedSets(d, e.key);
      saveDraft(); renderFormExercises(wrap);
    }));
    $$('[data-reason]', wrap).forEach((inp) => inp.addEventListener('input', (ev) => { d.exercises[+inp.dataset.reason].skipReason = ev.target.value; saveDraft(); }));
  ...
  function saveSession() {
    const d = draft;
    const filled = d.exercises.some((e) => !e.skipped && e.sets.some((st) => st.v != null && st.v !== ''));
    if (!filled && d.type !== 'rest') { toast(d.exercises.length && d.exercises.every((e) => e.skipped) ? 'Все блоки пропущены — тогда лучше отметить день отдыха' : 'Внесите хотя бы один подход'); return; }
    const session = {
      ...
      exercises: d.exercises.map((e) => (e.skipped
        ? { key: e.key, unit: e.unit, weighted: e.weighted, skipped: true, skipReason: (e.skipReason || '').trim(), sets: [] }
        : {
    ...
    const row = session.exercises.find((e) => e.key === 'row' && !e.skipped && e.sets.length);
  ...
  function sessionTableHtml(s) {
    ...
    return `<div class="session-body"><table>${s.exercises.map((e) => {
      ...
      if (e.skipped) return `<tr class="skipped"><td>${esc(ex.name)}</td><td>пропущено${e.skipReason ? ' · ' + esc(e.skipReason) : ''}</td></tr>`;
  ...
  function renderSettings() {
    ...
    const misc = card(`<h1>Приложение</h1>
      <dl class="kv"><dt>Тренировок записано</dt><dd>${state.sessions.length}</dd><dt>Дневник ведётся с</dt><dd>${esc(fmtShort(state.createdAt || todayStr()))}</dd><dt>Оффлайн-режим</dt><dd id="sw-status">${'serviceWorker' in navigator ? 'проверяем…' : 'недоступен в этом браузере'}</dd><dt>Версия</dt><dd>${esc(APP_VERSION.indexOf('__') === 0 ? 'локальная сборка' : APP_VERSION)}</dd></dl>
      <div class="btn-row" style="margin-top:8px"><button class="btn btn-ghost btn-sm" type="button" id="btn-update">Проверить обновления</button></div>
    ...
    updateSwStatus();
    $('#btn-update', misc).addEventListener('click', checkForUpdate);
  ...
  function offerUpdate() {
    const t = $('#toast');
    t.innerHTML = 'Доступна новая версия. <button type="button" class="btn btn-sm" id="btn-reload" style="margin-left:8px">Обновить</button>';
  ...
  function checkForUpdate() {
    if (!swReg) { toast(location.protocol === 'file:' ? 'Файл открыт локально: скачайте новую версию файла' : 'Оффлайн-режим не включён — просто перезагрузите страницу'); return; }
    toast('Проверяем…');
    swReg.update().then(() => { setTimeout(() => { if (!(swReg.installing || swReg.waiting)) toast('У вас последняя версия'); }, 1500); }).catch(() => toast('Нет сети — проверим позже'));
  }
  if ('serviceWorker' in navigator && (location.protocol === 'https:' || location.hostname === 'localhost' || location.hostname === '127.0.0.1')) {
    navigator.serviceWorker.register('sw.js').then((reg) => {
      swReg = reg; swState = 'включён'; updateSwStatus();
      if (reg.waiting && navigator.serviceWorker.controller) offerUpdate();
      reg.addEventListener('updatefound', () => {
        const nw = reg.installing;
        nw && nw.addEventListener('statechange', () => { if (nw.state === 'installed' && navigator.serviceWorker.controller) offerUpdate(); });
      });
      // Проверять обновления при каждом возврате в приложение
